...
interface UseIndexedDBResult {
	...
	updateValue: (params: { tableName: string; id: number; newItem: any }) => void;
	deleteValue: (tableName: string, id: number) => number | null;
	deleteAll: (tableName: string) => void;
	isDBConnecting: boolean;
}

export const useIndexedDB = (databaseName: string, tableNames: string[]): UseIndexedDBResult => {
	...
	const putValue = (tableName: string, value: object): Promise<IDBValidKey | null> => {
		return new Promise((resolve, reject) => {
			try {
	...
	const putBulkValue = async (tableName: string, values: object[]): Promise<any[]> => {
		try {
			const store = getTransaction(tableName, 'readwrite');
	...
	const updateValue = ({ tableName, id, newItem }: { tableName: string; id: number; newItem: any }) => {
		try {
			const store = getTransaction(tableName, 'readwrite');
			const request = store.get(id);
			request.onsuccess = () => {
